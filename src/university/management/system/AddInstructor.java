...
public class AddInstructor extends JFrame implements ActionListener {
    ...
    JLabel heading,lblname,lblfname,lblempID,labelempID,dob,lblAddress,lblPhone,lblEAddress,lblAddress1,lblXMarks,lblXIIMarks,lblXIIMarks1,lblAdhaar,lblQfc,lblBranch;
    JTextField tfname,tffname,tfAddress,tfEAddress,tfXMarks,tfXIIMarks;
    static JTextField tfAdhaar,tfPhone;
    JComboBox cbqfc,cbbranch;
    JDateChooser dcdob;
    JButton submit, cancel;
    
    Random random= new Random();
    // Generates a random number between 70000000 and 79999999
    ...
    String result = String.valueOf(randomLetter) + randomNumber;
    ...
    AddInstructor(){
        ...
        lblempID.setBounds(50,200,190,30);
        lblempID.setFont(new Font("Lato",Font.PLAIN,20));
        add(lblempID);
        
        // Because we want non-editable field -> so JLabel and not JTextfield
        labelempID = new JLabel(result);
        labelempID.setBounds(200, 195, 150, 40);
        labelempID.setFont(new Font("Lato",Font.PLAIN,18));
        add(labelempID);
        ...
        String[] qfc = {"B.Tech","B.E","BBA","MBA","BA","MA","B.Sc","M.Sc"};
        cbqfc = new JComboBox(qfc);
        cbqfc.setBounds(190,460,150,30);
        cbqfc.setFont(new Font("Lato",Font.PLAIN,14));
        cbqfc.setBackground(Color.WHITE);
        add(cbqfc);
        
        //Branch
        lblBranch = new JLabel("Department: ");
    ...
    private static void validateAdhaar(String adhaar){
        boolean isValid = adhaar.length() == 12;
        if (isValid){
    ...
    private static void validatePhone(String number){
        boolean isValid = number.length() == 10;
        if (isValid){
    ...
    public void actionPerformed(ActionEvent ae){
        try {
            ...
            if (ae.getSource() == submit){
                ...
                String course =(String) cbqfc.getSelectedItem();
                String branch = (String) cbbranch.getSelectedItem();
                
                try {
                    
                    String query = "INSERT INTO teacher VALUES ('"+name+"','"+fname+"','"+roll+"','"+doB+"','"+address+"','"+phone+"','"+email+"','"+X+"','"+XII+"','"+adhaar+"','"+course+"','"+branch+"')";
                    
                    Conn c = new Conn();
                    //DML command
                    c.s.executeUpdate(query);
                    
                    JOptionPane.showMessageDialog(null,"Instructor Details inserted Successfully");
                    setVisible(false);
